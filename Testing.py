...
def bigtest4H():
    testing = pandas.DataFrame(columns=['Stat', 'Contact', 'Power', 'Vision'])
    hitters = [(0, 0, 0), (5, 5, 5), (10, 10, 10)]
    for i in range(11):
        pitchers = [(i, 5, 5), (5, i, 5), (5, 5, i)]
        testing.loc[i] = [str(i), test4(pitchers[0], hitters[1]), test4(pitchers[1], hitters[1]), test4(pitchers[2], hitters[1])]
    testing.set_index('Stat', inplace=True)
    print(testing)


def bigtest4P():
    testing = pandas.DataFrame(columns=['Stat', 'Control', 'Velocity', 'Movement'])
    hitters = [(0, 0, 0), (5, 5, 5), (10, 10, 10)]
    for i in range(11):
        pitchers = [(i, 5, 5), (5, i, 5), (5, 5, i)]
        testing.loc[i] = [str(i), test4(hitters[1], pitchers[0]), test4(hitters[1], pitchers[1]),
                          test4(hitters[1], pitchers[2])]
    testing.set_index('Stat', inplace=True)
    print(testing)
...
def test4(hScore, pScore):  # Sims a bunch of innings and returns a stat like ERA or OPS (currently ERA)
    ...
    board = Scoreboard('STL', 0)  # Busch Field is just like a fairly average park
    hitter = Hitter(nameGen(), 'H ')
    (hitter.con, hitter.pow, hitter.vis) = hScore
    (hitter.field, hitter.speed) = (5, 5)
    oLineup = Lineup(None, None, None)
    oLineup.dAlign = [hitter]*10
    for i in range(9):
        oLineup.battingOrder.loc[len(oLineup.battingOrder)] = [hitter, 'H ', hScore*3, hitter.OPS]
